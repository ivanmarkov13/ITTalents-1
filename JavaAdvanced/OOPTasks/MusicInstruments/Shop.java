...
public class Shop {
	...
	private void addInstrument(String type, MusicInstrument instr){
		...
		if(!products.containsKey(type)){
			//if not - put the type as a key and initialize new arraylist with instruments;
			...
			for (MusicInstrument m : instr.getValue()) {
				...
				if (m.getName().equals(name)) {
					...
					if(m.getQuantity()>=units) {
						//if yes - reduce quantity, increase product's number of sales and add money to shop's money
	...
	public void printCatalog() {
		...
		// a private method that sorts the instruments with given comparator and prints the sorted catalgoue
		private void printSortedInstruments(Comparator<MusicInstrument>comp ) {
			//put all instruments in treeSet and sort them with the given comparator
		...
		//methods that call the above method(for sorting and printing) with different comparators;
		...
		public void printAvailableInstruments() {
			//iterate over products and check if the instrument quantity is positive
			for (Entry<String, ArrayList<MusicInstrument>> instr : products.entrySet()) {
		...
		//a method that creates a new collection that keep all instruments with their number of sales
		private LinkedList<Entry<MusicInstrument, Integer>> putInstrumetsWithTheirNumberOfSalesInList () {
			//create hashmap instrument -> numberofsales and put all instruments and their number of sales
			...
			//create a list (for later to sort and iterate) of entries and return it
			LinkedList<Entry<MusicInstrument, Integer>> list = new LinkedList<>(soldInstruments.entrySet());
			return list;
		}
		
		
		//method that calls the above method and sorts the instruments by number of sales
		public void printInfoForSoldInstruments() {
			LinkedList<Entry<MusicInstrument, Integer>> list = new LinkedList<>(putInstrumetsWithTheirNumberOfSalesInList());
			Collections.sort(list, new Comparator<Entry<MusicInstrument, Integer>>() {
		...
		//use the same method from above and calculate the accumulated money from sales
		...
		public void printInfoForLeastSoldInstrument() {
			...
			System.out.println("The least sold instrument is " + LeastSoldInstrument.getKey() + ", sold " + LeastSoldInstrument.getValue() + " times");
			
		}
		
		
		//a method that finds the type of instruments with greatest number of sales; this method uses the original collection(products) cause it keeps the type of instruments
		public void printInfoForBestSellerTypeOfInstrumentsBySoldUnits () {
			//create a list of entries and iterate to find the entry(type->instruments) with the greatest numbed of sales
			LinkedList<Entry<String, ArrayList<MusicInstrument>>> list = new LinkedList<Entry<String, ArrayList<MusicInstrument>>>(products.entrySet());
			...
			System.out.println("Best seller type by number of sales is " + bestSellerType.getKey() + " with total number of sales - " + greatestNumberOfSales);
		...
		public void printInfoForBestSellerTypeOfInstrumentsByAccumulatedMoney () {
			...
			System.out.println("Best seller type by accumulated money is " + bestSellerType.getKey() + " with total number of sales - " + greatestSumFromSales);
